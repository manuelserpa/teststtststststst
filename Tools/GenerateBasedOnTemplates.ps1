...
function Get-Title {
	...
	$indexOfNewLine = $title.indexof("`n")
...
foreach ( $templateFile in $templateFiles ) {
	...
	foreach( $file in $files ) 
	{
		...
		$folderPath = $file.FullName.substring(0, $file.FullName.lastindexof("\"))
		$folderPath = $folderPath.substring($folderPath.indexof("assets\") + "assets\".length)
		$folderPath = $folderPath -replace "\\", ">"
		#Write-Host "$folderPath"
		#exit
		$link = "/Imec/$folderPath>$fileName"
		...
		if ( $isIndexMode ) {
			...
			if ( $isTableMode ) {
				...
				$output += "| [$title]($link) | $description |`n"
			}
		}
	}

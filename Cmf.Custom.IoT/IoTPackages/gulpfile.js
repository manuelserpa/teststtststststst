...
gulp.task("build", function (callback) {
  if (pluginYargs.server) {
    ...
    rootUtils.runOperation(
      ...
      "build",
      callback,
      typeof _framework === "string" && _framework !== "",
      isWebAppCompilable
    );
  } else {
    var ops = [];
    if (pluginYargs.parallelBuild !== false) {
      ...
      if (Number.isInteger(pluginYargs.parallelBuild)) {
        pluginUtil.log(
          pluginUtil.colors.yellow(
            `Using ${pluginYargs.parallelBuild} tasks in parallel`
          )
        );
        // split in tasks
        ...
        for (
          i = 0;
          i < opsToSliceArray.length;
          i += pluginYargs.parallelBuild
        ) {
  ...
  Promise.allSettled(pkgPromises).then((results) => {
    ...
    for (const result of results) {
      if (
        ...
        typeof result.reason.stderr != "undefined" &&
 ...
 */
gulp.task("apps", function (callback) {
  pluginRunSequence.apply(this, [
    "build",
    `${_config.webAppPrefix}.web>build`,
    callback,
  ]);
});
...
gulp.task("start", function (callback) {
  pluginRunSequence(`${_config.webAppPrefix}.web>start`);
});
...
gulp.task("test", function (callback) {
  pluginRunSequence.apply(this, applyOps("test").concat(callback));
});
...
gulp.task("clean", function (callback) {
  pluginRunSequence.apply(this, applyOps("clean").concat(callback));
});

gulp.task("clean-libs", function (callback) {
  pluginRunSequence.apply(this, applyOps("clean-libs").concat(callback));
});

gulp.task("purge", function (callback) {
  pluginRunSequence.apply(this, applyOps("purge").concat(callback));
});

gulp.task("watch", function (callback) {
  pluginRunSequence.apply(this, applyOps("watch").concat(callback));
});

/**
 * i18n generators
 */
gulp.task("create-missing-i18n", function (callback) {
  pluginRunSequence.apply(
    this,
    applyOps("create-missing-i18n").concat(callback)
  );
});
gulp.task("i18n-ts2po", function (callback) {
  pluginRunSequence.apply(this, applyOps("i18n-ts2po").concat(callback));
});
gulp.task("i18n-po2ts", function (callback) {
  pluginRunSequence.apply(this, applyOps("i18n-po2ts").concat(callback));
});
...
gulp.task("publish", function (callback) {
  pluginRunSequence.apply(this, applyOps("publish").concat(callback));
});

gulp.task("check-version", function (callback) {
  pluginRunSequence.apply(this, applyOps("check-version").concat(callback));
});
gulp.task("set-version", function (callback) {
  pluginRunSequence.apply(this, applyOps("set-version").concat(callback));
});
gulp.task("npm-dist-tag-add", function (callback) {
  pluginRunSequence.apply(this, applyOps("npm-dist-tag-add").concat(callback));
});
gulp.task("npm-dist-tag-rm", function (callback) {
  pluginRunSequence.apply(this, applyOps("npm-dist-tag-rm").concat(callback));
});
gulp.task("npm-dist-tag-ls", function (callback) {
  pluginRunSequence.apply(this, applyOps("npm-dist-tag-ls").concat(callback));
});
gulp.task("npm-dist-tag-copy-version", function (callback) {
  pluginRunSequence.apply(
    this,
    applyOps("npm-dist-tag-copy-version").concat(callback)
  );
});

gulp.task("bump-version", function (callback) {
  pluginRunSequence.apply(this, applyOps("bump-version").concat(callback));
});
gulp.task("generate-package-lock", function (callback) {
  pluginRunSequence.apply(
    this,
    applyOps("generate-package-lock").concat(callback)
  );
});

gulp.task("check-circular-imports", function (callback) {
  pluginRunSequence.apply(
    this,
    applyOps("check-circular-imports").concat(callback)
  );
});

...
import {
    RetrieveMaterialPropertyTask,
    RetrieveMaterialPropertySettings
} from "../../../../src/tasks/retrieveMaterialProperty/retrieveMaterialProperty.task";
import CheckOrderPropertyChangeTaskModule from "../../../../src/tasks/retrieveMaterialProperty/index";

describe("CheckOrderPropertyChange Task tests", () => {


    // Optional: See container handling under checkOrderPropertyChangeTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const checkOrderPropertyChangeTestFactory = (  settings: RetrieveMaterialPropertySettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: CheckOrderPropertyChangeTaskModule,
            id: "0",
            settings: settings || <RetrieveMaterialPropertySettings>{
                // message: ""
            }
        };
     ...
     *    0 - CheckOrderPropertyChange Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the CheckOrderPropertyChange task
     * All Outputs of CheckOrderPropertyChange Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the CheckOrderPropertyChange to be activated
     * and check the changes to see if the CheckOrderPropertyChange task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        checkOrderPropertyChangeTestFactory(undefined,

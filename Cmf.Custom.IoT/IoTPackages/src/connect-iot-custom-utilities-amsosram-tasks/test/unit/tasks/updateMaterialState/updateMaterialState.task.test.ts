...
import {
    UpdateMaterialStateTask,
    UpdateMaterialStateSettings
} from "../../../../src/tasks/updateMaterialState/updateMaterialState.task";
import DynamicEventHandlerTaskModule from "../../../../src/tasks/updateMaterialState/index";

describe("DynamicEventHandler Task tests", () => {
    ...
    // Optional: See container handling under dynamicEventHandlerTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const dynamicEventHandlerTestFactory = (  settings: UpdateMaterialStateSettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: DynamicEventHandlerTaskModule,
            id: "0",
            settings: settings || <UpdateMaterialStateSettings>{
             //   message: ""
            }
        };
     ...
     *    0 - DynamicEventHandler Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the DynamicEventHandler task
     * All Outputs of DynamicEventHandler Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the DynamicEventHandler to be activated
     * and check the changes to see if the DynamicEventHandler task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        dynamicEventHandlerTestFactory(undefined,

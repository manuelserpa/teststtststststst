...
import {
    ChangeWaferContainerTask,
    ChangeWaferContainerSettings
} from "../../../../src/tasks/changeWaferContainer/changeWaferContainer.task";
import ChangeWaferContainerTaskModule from "../../../../src/tasks/changeWaferContainer/index";

describe("ChangeWaferContainer Task tests", () => {
    ...
    // Optional: See container handling under changeWaferContainerTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const changeWaferContainerTestFactory = (  settings: ChangeWaferContainerSettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: ChangeWaferContainerTaskModule,
            id: "0",
            settings: settings || <ChangeWaferContainerSettings>{
                message: ""
            }
        };
     ...
     *    0 - ChangeWaferContainer Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the ChangeWaferContainer task
     * All Outputs of ChangeWaferContainer Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the ChangeWaferContainer to be activated
     * and check the changes to see if the ChangeWaferContainer task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        changeWaferContainerTestFactory(undefined,

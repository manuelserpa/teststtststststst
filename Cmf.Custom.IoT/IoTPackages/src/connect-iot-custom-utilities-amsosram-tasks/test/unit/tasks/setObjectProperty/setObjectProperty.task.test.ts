...
import {
    SetObjectPropertyTask,
    SetObjectPropertySettings
} from "../../../../src/tasks/setObjectProperty/setObjectProperty.task";
import CompleteRecipeOnEquipmentTaskModule from "../../../../src/tasks/setObjectProperty/index";

describe("CompleteRecipeOnEquipment Task tests", () => {
    ...
    // Optional: See container handling under completeRecipeOnEquipmentTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const completeRecipeOnEquipmentTestFactory = (  settings: SetObjectPropertySettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: CompleteRecipeOnEquipmentTaskModule,
            id: "0",
            settings: settings || <SetObjectPropertySettings>{
               // message: ""
            }
        };
     ...
     *    0 - CompleteRecipeOnEquipment Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the CompleteRecipeOnEquipment task
     * All Outputs of CompleteRecipeOnEquipment Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the CompleteRecipeOnEquipment to be activated
     * and check the changes to see if the CompleteRecipeOnEquipment task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        completeRecipeOnEquipmentTestFactory(undefined,

...
import {
    RetrieveSubMaterialPropertyTask,
    RetrieveSubMaterialPropertySettings
} from "../../../../src/tasks/retrieveSubMaterialProperty/retrieveSubMaterialProperty.task";
import CompleteOnProducedHandlerTaskModule from "../../../../src/tasks/retrieveSubMaterialProperty/index";

describe("CompleteOnProducedHandler Task tests", () => {
    ...
    // Optional: See container handling under completeOnProducedHandlerTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const completeOnProducedHandlerTestFactory = (  settings: RetrieveSubMaterialPropertySettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: CompleteOnProducedHandlerTaskModule,
            id: "0",
            settings: settings || <RetrieveSubMaterialPropertySettings>{
          //      message: ""
            }
        };
     ...
     *    0 - CompleteOnProducedHandler Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the CompleteOnProducedHandler task
     * All Outputs of CompleteOnProducedHandler Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the CompleteOnProducedHandler to be activated
     * and check the changes to see if the CompleteOnProducedHandler task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        completeOnProducedHandlerTestFactory(undefined,

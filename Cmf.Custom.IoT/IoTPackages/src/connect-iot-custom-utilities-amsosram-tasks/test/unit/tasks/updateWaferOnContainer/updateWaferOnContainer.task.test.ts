...
import {
    UpdateWaferOnContainerTask,
    UpdateWaferOnContainerSettings
} from "../../../../src/tasks/updateWaferOnContainer/updateWaferOnContainer.task";
import UpdateWaferOnContainerTaskModule from "../../../../src/tasks/updateWaferOnContainer/index";

describe("UpdateWaferOnContainer Task tests", () => {
    ...
    // Optional: See container handling under updateWaferOnContainerTestFactory
    // let dataStoreMock: DataStoreMock;
    beforeEach(() => {
        // dataStoreMock = new DataStoreMock();
    });

    const updateWaferOnContainerTestFactory = (  settings: UpdateWaferOnContainerSettings | undefined,
                                            trigger: Function,
                                            validate: Function): void => {

        const taskDefinition = {
            class: UpdateWaferOnContainerTaskModule,
            id: "0",
            settings: settings || <UpdateWaferOnContainerSettings>{
                message: ""
            }
        };
     ...
     *    0 - UpdateWaferOnContainer Task
     *    1 - Mockup task
     *
     * All Outputs of Mock task are connected to the inputs of the UpdateWaferOnContainer task
     * All Outputs of UpdateWaferOnContainer Task are connected to the Mock task inputs
     *
     * You, as the tester developer, will trigger the outputs necessary for the UpdateWaferOnContainer to be activated
     * and check the changes to see if the UpdateWaferOnContainer task sent you the correct values
     *
    ...
    it("should get success when activated", (done) => {
        updateWaferOnContainerTestFactory(undefined,

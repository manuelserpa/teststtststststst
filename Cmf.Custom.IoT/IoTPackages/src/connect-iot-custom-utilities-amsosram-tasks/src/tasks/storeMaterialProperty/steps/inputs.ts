...
import {  StoreMaterialPropertyInputSettings} from "../storeMaterialProperty.task";
...
@Component({
    moduleId: module.id,
    selector: "connect-iot-controller-engine-core-tasks-storeMaterialProperty-settings-input",
...
export class Inputs extends CoreComponent implements ng.OnInit, ng.OnChanges {
    ...
    public data: StoreMaterialPropertyInputSettings[] = null;

    /* On Data Change emitter.*/
    public dataChange: ng.EventEmitter<StoreMaterialPropertyInputSettings[]> = new ng.EventEmitter<StoreMaterialPropertyInputSettings[]>();
    ...
    private buildColumnViewModel(data: StoreMaterialPropertyInputSettings[]): void {
        this._model = {
            rootNode: {
                id: "root",
                name: "links",
    ...
    private buildColumnViewChildModel(data: StoreMaterialPropertyInputSettings): ColumnViewModel.ColumnViewLeaf {
        return {
    ...
    private validateRow(context: OnValidateArgs, data: StoreMaterialPropertyInputSettings): boolean {
        if (data.name != null && data.name.length > 0 && data.path != null && data.valueType != null) {
            return true;
        } else {
            if (context) {
     ...
     */
    public getLinks(): StoreMaterialPropertyInputSettings[] {
        let rt: StoreMaterialPropertyInputSettings[] = [];

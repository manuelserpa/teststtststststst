...
import i18n from "./i18n/retrieveSubMaterialProperty.default";
 ...
 *
 * ### Settings
 * See {@see RetrieveSubMaterialPropertySettings}
 */
@Task.Task({
    name: i18n.TITLE,
...
export class RetrieveSubMaterialPropertyTask implements Task.TaskInstance, RetrieveSubMaterialPropertySettings {
    ...
    /** Settings */
    /** Properties Settings */
    public outputs: RetrieveSubMaterialPropertyOutputSettings[];
        ...
        if (changes["activate"]) {
            ...
            try {
                ...
                if (this.subMaterialName != null && this.subMaterialName !== "") {
                    subMaterial = material.SubMaterials.find(o => o.MaterialName === this.subMaterialName)
                    ...
                    if (subMaterial == null) {
                        throw new Error (
                            `Error trying to get a submaterial property: the submaterial with state '
                            ${this.subMaterialState}' does not exist on the persistence`)
                    }
                } else {
                    throw new Error (
                        `Error: No valid information to retrieve material from persistence`)
                    ...
                    if (this.outputs != null) {
                        this.outputs.forEach(output => {
                            const value = this.retrieveSubMaterialProperty(subMaterial, output.path);
   ...
   private retrieveSubMaterialProperty(material: any, path: string ): string {
        const parts = path.split( "." );
        let property = material;

        for (let i = 0; i < parts.length; i++) {
            if (property == null) {
                throw new Error(`The property '${path}' does not exist on the subMaterial object`);
            }
...
// Add settings here
/** RetrieveSubMaterialProperty Settings object */
export interface RetrieveSubMaterialPropertySettings {
...
export interface RetrieveSubMaterialPropertyOutputSettings {
    /** Output name */
    name: string;

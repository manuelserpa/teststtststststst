...
import * as CustomDataTraceTask from "./customDataTrace.task";
...
/** i18n */
import i18n from "./i18n/customDataTrace.settings.default";
...
@Component({
    moduleId: module.id,
    selector: "connect-iot-controller-engine-core-tasks-customDataTrace-settings",
    templateUrl: "customDataTrace.settings.html",
    assign: {
        i18n: i18n,
        OutputTypeEnum: CustomDataTraceTask.CustomDataTraceOutputType
    }
})
export class CustomDataTraceSettings extends TaskSettingsBase implements ng.OnChanges, OnValidate, ng.OnInit {
     ...
     * Settings object
     */
    public settings: CustomDataTraceTask.CustomDataTraceSettings;
     ...
     */
    private _buildColumnViewModel(outputs: CustomDataTraceTask.CustomDataTraceOutputSettings[]): void {
     ...
     */
    private _buildColumnViewChildModel(output: CustomDataTraceTask.CustomDataTraceOutputSettings): ColumnViewModel.ColumnViewLeaf {
        return {
            id: this.framework.sandbox.util.uniqueId("output_"),
            name: output.property ? output.property.Name : i18n.NOT_SPECIFIED,
            value: output.outputType != null ? CustomDataTraceTask.CustomDataTraceOutputType[output.outputType] : "",
            canRemove: true,
            tag: output
        };
    }
    ...
    public onValidate(context: OnValidateArgs, model?: ValidatorModel): Promise<boolean> {
        return model.validate(context).then(result => {
            ...
            if (result === true) {
                ...
                const output = <CustomDataTraceTask.CustomDataTraceOutputSettings>this._selectedOutput.rootNode.tag;
                ...
                if (output?.property != null && output?.outputType != null) {
                    ...
                    if (output.property.IsReadable !== false || this.settings?.allowNonReadable === true) {
                        ...
                        isRepeated = (<ColumnViewModel.ColumnViewLeaf[]>this._outputsModel.rootNode.children).some(node => {
                            const outputToCompare = (<CustomDataTraceTask.CustomDataTraceOutputSettings>node.tag);
    ...
    public _onColumnViewAdd(args: ColumnViewAddArgs): void {
        ...
        (<ColumnViewModel.ColumnViewLeaf[]>node.children).push(this._buildColumnViewChildModel({
            property: null,
            outputType: CustomDataTraceTask.CustomDataTraceOutputType.Value
        }));
    ...
    public _onOutputChange(value: any, destPath: string): void {
        ...
        if (destPath === "outputType") {
            ...
            ? "" : CustomDataTraceTask.CustomDataTraceOutputType[value];
        }
    }
     ...
     */
    public onBeforeSave(settings: CustomDataTraceTask.CustomDataTraceSettings): CustomDataTraceTask.CustomDataTraceSettings {
        // outputs
...
@Module({
    ...
    declarations: [CustomDataTraceSettings],
    defaultRoute: CustomDataTraceSettings
})
export class CustomDataTraceSettingsModule {
}

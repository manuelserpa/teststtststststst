...
import i18n from "./i18n/createContainer.default";
import { ContainerProcessHandler } from "../../persistence/implementation/containerDataHandler";

/** Default values for settings */
export const SETTINGS_DEFAULTS: CreateContainerSettings = {
    inputs: [],
    outputs: [],
    message: ""
}
 ...
 *
 * ### Settings
 * See {@see CreateContainerSettings}
 */
@Task.Task({
    name: i18n.TITLE,
...
export class CreateContainerTask implements Task.TaskInstance, CreateContainerSettings {
    ...
    async onChanges(changes: Task.Changes): Promise<void> {
        if (changes["activate"]) {
            ...
            try {
                ...
                if (container) {
                    container = await this._containerProcess.updateContainer(this.containerId, this.loadPort, this.slotMap, null);
                } else {
...
// Add settings here
/** CreateContainer Settings object */
export interface CreateContainerSettings extends TaskDefaultSettings {
    message: string;
}

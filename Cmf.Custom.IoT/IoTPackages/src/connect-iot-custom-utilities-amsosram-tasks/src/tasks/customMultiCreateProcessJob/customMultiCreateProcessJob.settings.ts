...
import * as CustomMultiCreateProcessJobTask from "./customMultiCreateProcessJob.task";
/** Angular */
...
/** i18n */
import i18n from "./i18n/customMultiCreateProcessJob.settings.default";
import { ComplexValueType } from "@criticalmanufacturing/connect-iot-controller-engine/src/system";
import { RecipeSpecificationType } from "./customMultiCreateProcessJob.task";

/** Constants */
export interface CustomMultiCreateProcessJobTaskSettings extends
    CustomMultiCreateProcessJobTask.CustomMultiCreateProcessJobSettings, WorkflowModel.TaskDefinitionSettings { }
...
@Component({
    moduleId: module.id,
    selector: "connect-iot-controller-engine-core-tasks-customMultiCreateProcessJob-settings",
    templateUrl: "customMultiCreateProcessJob.settings.html",
    assign: {
        i18n: i18n,
        RecipeSpecificationType: RecipeSpecificationType
    }
})
export class CustomMultiCreateProcessJobSettings extends TaskSettingsBase implements ng.OnInit {

    /**
     * Task settings
     */
    public settings: CustomMultiCreateProcessJobTaskSettings;

    /** helper to access main task (for defaults loading) */
    private _taskInstance: CustomMultiCreateProcessJobTask.CustomMultiCreateProcessJobTask;
...
@Module({
    ...
    declarations: [CustomMultiCreateProcessJobSettings],
    defaultRoute: CustomMultiCreateProcessJobSettings
})

export class CustomMultiCreateProcessJobSettingsModule { }
